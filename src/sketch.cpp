...
#define ALICE_PIPELINE_TEST

#ifdef ALICE_PIPELINE_TEST

Alice::Buffer<V3> g_TerrainMesh;
Alice::Buffer<V3> g_QuadMesh;
Alice::Buffer<V3> g_OSMMesh;
...
// Sweep Results Logging
void logSweep(const char* message)
{
    printf("[SWEEP] %s\n", message);
    FILE* f = nullptr;
...
ZTestPlane g_ZPlanes[2];

extern "C" void setup()
{
    logSweep("--- Starting Alice Viewer 2 Pipeline Sweep ---");
    // Ensure arena is initialized before loading meshes
    ...
    // Initial Load - Use fixed RhinoLoader
...
void drawMesh(const Alice::Buffer<V3>& buffer, V3 color)
{
    if (buffer.size() == 0) return;
