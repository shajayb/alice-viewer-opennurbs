#ifndef CESIUM_GEPR_AND_OSM_H
#define CESIUM_GEPR_AND_OSM_H
...
namespace CesiumNetwork {
    ...
    static void FetchAsync(const char* u, size_t maxBytes, const char* bt, std::function<void(long, const FetchBuffer&)> cb) {
        ...
        curl_easy_setopt(req->handle, CURLOPT_TIMEOUT, 15L);
        curl_easy_setopt(req->handle, CURLOPT_CONNECTTIMEOUT, 5L);
        req->headers = NULL;
        if (bt && bt[0]) {
    ...
    static void UpdateAsync() {
        ...
        std::vector<CURL*> completed_handles;
        while ((msg = curl_multi_info_read(g_Multi, &msgs_left))) {
            if (msg->msg == CURLMSG_DONE) {
                completed_handles.push_back(msg->easy_handle);
            }
        }
        
        for (CURL* e : completed_handles) {
...
namespace CesiumGeprAndOsm {
    using namespace CesiumMath;

    enum TestState { STATE_STREAMING, STATE_AGGREGATE, STATE_LOAD_CACHED, STATE_CACHED_STENCIL_OSM, STATE_CACHED_STENCIL_OSM_WAIT, STATE_CACHED_STENCIL_GEPR, STATE_CACHED_STENCIL_GEPR_WAIT, STATE_VERIFY, STATE_DONE };
    ...
    static TestView g_Views[4] = {
        ...
        { {0, 0, 0}, 250.0f, 0.4f, 1.57f },     // 1: Side environment
        { {0, 0, 0}, 400.0f, 0.6f, -0.78f },    // 2: Wide Angle Aerial
        { {0, 0, 0}, 200.0f, 0.2f, 3.14f }      // 3: Back Angle
    };
    ...
    struct TestLocation {
        ...
        double alt;
        char semantic[256];
        char generated_description[512];
        bool export_osm_images;
        bool export_gepr_images;
        bool export_osm_3d;
        bool export_gepr_3d;
    };
    ...
    static void freeResource(RenderResources* res) {
        if (!res) return;
        // Do not delete GL objects, just recycle them when the pool element is reused
        res->inUse = false;
    }
    ...
    struct Tileset {
        ...
        int traverse(Tile* node, int depth) {
            ...
            if (node->childrenCount > 0 && node->contentUri[0] == '\0') shouldRefine = true; // Force traverse deeper for OSM empty placeholders
            if (node->childrenCount > 0 && (!node->isLoaded || !node->payload)) shouldRefine = true; // ALWAYS refine if we have no payload to show!
            ...
            if (node->contentUri[0] != '\0' && !node->isLoaded && !node->isLoading) {
                if (CesiumNetwork::g_AsyncRequests.size() < 40) {
                    node->isLoading = true;
                    char url[1024];
    ...
    static GLuint g_CachedGeprVAO = 0, g_CachedGeprVBO = 0, g_CachedGeprEBO = 0;
    static int g_CachedGeprCount = 0;
    static GLuint g_CachedOsmVAO = 0, g_CachedOsmVBO = 0, g_CachedOsmEBO = 0;
    static int g_CachedOsmCount = 0;
        ...
        if (f.is_open()) {
            ...
            if (j.contains("locations")) {
                ...
                for (size_t i=0; i<locs.size(); ++i) {
                    ...
                    if (locs[(uint32_t)i].contains("generated_description")) {
                        strncpy(loc.generated_description, locs[(uint32_t)i]["generated_description"].get<std::string>().c_str(), 511);
                    } else {
                        loc.generated_description[0] = '\0';
                    }
                    loc.export_osm_images = locs[(uint32_t)i].contains("export_osm_images") && locs[(uint32_t)i]["export_osm_images"].type == AliceJson::J_BOOL ? locs[(uint32_t)i]["export_osm_images"].boolean : true;
                    loc.export_gepr_images = locs[(uint32_t)i].contains("export_gepr_images") && locs[(uint32_t)i]["export_gepr_images"].type == AliceJson::J_BOOL ? locs[(uint32_t)i]["export_gepr_images"].boolean : true;
                    loc.export_osm_3d = locs[(uint32_t)i].contains("export_osm_3d") && locs[(uint32_t)i]["export_osm_3d"].type == AliceJson::J_BOOL ? locs[(uint32_t)i]["export_osm_3d"].boolean : true;
                    loc.export_gepr_3d = locs[(uint32_t)i].contains("export_gepr_3d") && locs[(uint32_t)i]["export_gepr_3d"].type == AliceJson::J_BOOL ? locs[(uint32_t)i]["export_gepr_3d"].boolean : true;
        ...
        if (node->mesh) {
            for (size_t i=0; i<node->mesh->primitives_count; ++i) {
                ...
                if (p->type == cgltf_primitive_type_triangle_strip) {
                    ...
                    size_t icount = p->indices ? p->indices->count : pa->count;
                    if (icount >= 3) {
                        uint32_t i0 = p->indices ? (uint32_t)cgltf_accessor_read_index(p->indices, 0) : 0;
                        for (size_t k = 1; k + 1 < icount; ++k) {
                            uint32_t i1 = p->indices ? (uint32_t)cgltf_accessor_read_index(p->indices, k) : (uint32_t)k;
                            uint32_t i2 = p->indices ? (uint32_t)cgltf_accessor_read_index(p->indices, k+1) : (uint32_t)(k+1);
                            ebo[eIdx++] = i0 + offset; ebo[eIdx++] = i1 + offset; ebo[eIdx++] = i2 + offset;
                        }
                    }
                } else {
    ...
    static void setup() {
        ...
        if (!Alice::g_JsonArena.memory) Alice::g_JsonArena.init(256 * 1024 * 1024); // 256MB for JSON ASTs
    ...
    static void update(float dt) {
        ...
        Frustum& f = g_GeprTileset.currentFrustum;
    ...
    static void draw() {
        ...
        // Evict stale tiles
        if (g_GeprTileset.root && (g_CurrentState == STATE_STREAMING)) g_GeprTileset.evictStaleTiles(g_GeprTileset.root);
        if (g_OsmTileset.root && (g_CurrentState == STATE_STREAMING)) g_OsmTileset.evictStaleTiles(g_OsmTileset.root);
        ...
        if (g_CurrentState == STATE_STREAMING) {
            ...
            if (g_CurrentState == STATE_STREAMING && frameIdx % 100 == 0) {
                printf("[DEBUG] STATE_STREAMING Check: frameIdx=%u, start=%u, GEPR=%u, OSM=%u\n", (uint32_t)frameIdx, (uint32_t)g_StateFrameStart, g_GeprTileset.renderListCount, g_OsmTileset.renderListCount);
                fflush(stdout);
            }
            if (g_CurrentState == STATE_STREAMING && frameIdx - g_StateFrameStart > 800 && (g_GeprTileset.renderListCount > 0 || g_OsmTileset.renderListCount > 0)) {
                unsigned char* px = (unsigned char*)malloc(w*h*3);
                
                if (g_Locations[g_CurrentLocationIndex].export_osm_images) {
                    glClearColor(1.0f, 1.0f, 1.0f, 1.0f); glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
                    glEnable(GL_POLYGON_OFFSET_FILL); glPolygonOffset(-1.0f, -1.0f); glPolygonMode(GL_FRONT_AND_BACK, GL_LINE); glUniform1i(locPass, 1);
                    for (uint32_t i=0; i<g_OsmTileset.renderListCount; ++i) { Tile* t = g_OsmTileset.renderList[i]; if (t->rendererResources) { glBindVertexArray(t->rendererResources->vao); glDrawElements(GL_TRIANGLES, t->rendererResources->count, GL_UNSIGNED_INT, 0); } }
                    glPolygonMode(GL_FRONT_AND_BACK, GL_FILL); glDisable(GL_POLYGON_OFFSET_FILL);
                    glReadPixels(0,0,w,h,GL_RGB,GL_UNSIGNED_BYTE,px); stbi_flip_vertically_on_write(1);
                    char path[256]; snprintf(path, 256, "build/bin/OUTPUT/osm_stream_loc%d_view%d.png", g_CurrentLocationIndex, g_CurrentViewIndex); stbi_write_png(path, w, h, 3, px, w*3);
                }

                if (g_Locations[g_CurrentLocationIndex].export_gepr_images) {
                    glClearColor(1.0f, 1.0f, 1.0f, 1.0f); glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
                    glUniform1i(locPass, 0);
                    for (uint32_t i=0; i<g_GeprTileset.renderListCount; ++i) { Tile* t = g_GeprTileset.renderList[i]; if (t->rendererResources) { glBindVertexArray(t->rendererResources->vao); glDrawElements(GL_TRIANGLES, t->rendererResources->count, GL_UNSIGNED_INT, 0); } }
                    glReadPixels(0,0,w,h,GL_RGB,GL_UNSIGNED_BYTE,px); stbi_flip_vertically_on_write(1);
                    char path[256]; snprintf(path, 256, "build/bin/OUTPUT/gepr_stream_loc%d_view%d.png", g_CurrentLocationIndex, g_CurrentViewIndex); stbi_write_png(path, w, h, 3, px, w*3);
                }
                free(px);
                
                printf("[Test] View %d STREAMING Complete. Moving to Aggregation.\n", g_CurrentViewIndex);
                g_CurrentState = STATE_AGGREGATE; g_StateFrameStart = frameIdx;
            }
        } 
        
        if (g_CurrentState == STATE_AGGREGATE) {
            auto writeCache = [&](Tileset& ts, const char* suffix) {
                ...
                char binPath[256]; snprintf(binPath, 256, "build/bin/OUTPUT/cache_%s_loc%d_view%d.bin", suffix, g_CurrentLocationIndex, g_CurrentViewIndex);
            ...
            if (g_Locations[g_CurrentLocationIndex].export_gepr_3d) writeCache(g_GeprTileset, "gepr");
            if (g_Locations[g_CurrentLocationIndex].export_osm_3d) writeCache(g_OsmTileset, "osm");
            
            if (g_CurrentViewIndex == 0) {
                auto write3DM = [&](Tileset& ts, const char* suffix, bool isOsm) {
                    char dmpPath[256]; snprintf(dmpPath, 256, "build/bin/OUTPUT/cache_%s_loc%d_view%d.3dm", suffix, g_CurrentLocationIndex, g_CurrentViewIndex);
                    ONX_Model model;
                    ...
                    for (uint32_t i = 0; i < ts.renderListCount; ++i) {
                        ...
                        if (!isOsm && distToOrigin > 1500.0f) continue;
                        if (!isOsm && t->geometricError > 50.0) continue;
                        
                        cgltf_options opt = {cgltf_file_type_glb}; opt.memory.alloc_func = cgltf_alloc; opt.memory.free_func = cgltf_free_cb;
                        cgltf_data* data = 0; uint8_t* glbPayload = t->payload; size_t glbSize = t->payloadSize;
                        DVec3 rtcCenter = {0,0,0};
                        if (glbSize >= 28 && memcmp(glbPayload, "b3dm", 4) == 0) {
                            uint32_t ftj = *(uint32_t*)(glbPayload + 12), ftb = *(uint32_t*)(glbPayload + 16);
                            uint32_t btj = *(uint32_t*)(glbPayload + 20), btb = *(uint32_t*)(glbPayload + 24);
                            if (ftj > 0) {
                                auto ft = AliceJson::parse(glbPayload + 28, ftj);
                                if (ft.contains("RTC_CENTER")) { auto rtc = ft["RTC_CENTER"]; rtcCenter = {(double)rtc[0], (double)rtc[1], (double)rtc[2]}; }
                            }
                            uint32_t headerLen = 28 + ftj + ftb + btj + btb;
                            if (headerLen < glbSize) { glbPayload += headerLen; glbSize -= headerLen; }
                        }
                    ...
                    int vCount = exportVBO.size() / 6; mesh->m_V.SetCount(vCount);
                    for (int i=0; i<vCount; i++) mesh->SetVertex(i, ON_3dPoint(exportVBO[i*6], exportVBO[i*6+1], exportVBO[i*6+2]));
                    int fCount = exportEBO.size() / 3; mesh->m_F.SetCount(fCount);
                ...
                if (g_Locations[g_CurrentLocationIndex].export_osm_3d) write3DM(g_OsmTileset, "osm", true);
                if (g_Locations[g_CurrentLocationIndex].export_gepr_3d) write3DM(g_GeprTileset, "gepr", false);
            }
        ...
        } else if (g_CurrentState == STATE_LOAD_CACHED || g_CurrentState == STATE_CACHED_STENCIL_OSM || g_CurrentState == STATE_CACHED_STENCIL_OSM_WAIT || g_CurrentState == STATE_CACHED_STENCIL_GEPR || g_CurrentState == STATE_CACHED_STENCIL_GEPR_WAIT) {
            if (g_CachedGeprVAO == 0) {
                auto loadCache = [&](const char* suffix, GLuint& vao, GLuint& vbo, GLuint& ebo, int& count) {
                    char binPath[256]; snprintf(binPath, 256, "build/bin/OUTPUT/cache_%s_loc%d_view%d.bin", suffix, g_CurrentLocationIndex, g_CurrentViewIndex);
                    FILE* f = fopen(binPath, "rb");
                    if (f) {
                        uint32_t vCount, iCount; fread(&vCount, sizeof(uint32_t), 1, f); fread(&iCount, sizeof(uint32_t), 1, f);
                        std::vector<float> vData(vCount); std::vector<uint32_t> iData(iCount);
                        fread(vData.data(), sizeof(float), vCount, f); fread(iData.data(), sizeof(uint32_t), iCount, f); fclose(f);
                        glGenVertexArrays(1, &vao); glBindVertexArray(vao);
                        glGenBuffers(1, &vbo); glGenBuffers(1, &ebo);
                        glBindBuffer(GL_ARRAY_BUFFER, vbo); glBufferData(GL_ARRAY_BUFFER, vData.size()*4, vData.data(), GL_STATIC_DRAW);
                        glEnableVertexAttribArray(0); glVertexAttribPointer(0, 3, GL_FLOAT, 0, 24, 0);
                        glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, ebo); glBufferData(GL_ELEMENT_ARRAY_BUFFER, iData.size()*4, iData.data(), GL_STATIC_DRAW);
                        count = (int)iCount;
                    }
                };
                loadCache("gepr", g_CachedGeprVAO, g_CachedGeprVBO, g_CachedGeprEBO, g_CachedGeprCount);
                loadCache("osm", g_CachedOsmVAO, g_CachedOsmVBO, g_CachedOsmEBO, g_CachedOsmCount);
            }
            GLint locPass = glGetUniformLocation(g_Program, "uPass");
            
            // Render depending on state
            bool drawOSM = (g_CurrentState == STATE_CACHED_STENCIL_OSM || g_CurrentState == STATE_CACHED_STENCIL_OSM_WAIT);
            bool drawGEPR = (g_CurrentState == STATE_CACHED_STENCIL_GEPR || g_CurrentState == STATE_CACHED_STENCIL_GEPR_WAIT);
            if (g_CurrentState == STATE_LOAD_CACHED) { drawOSM = true; drawGEPR = true; }
            
            if (drawGEPR && g_CachedGeprVAO) {
                glPolygonMode(GL_FRONT_AND_BACK, GL_FILL); glUniform1i(locPass, 0);
                glBindVertexArray(g_CachedGeprVAO); 
                glDrawElements(GL_TRIANGLES, g_CachedGeprCount, GL_UNSIGNED_INT, 0);
            }
            if (drawOSM && g_CachedOsmVAO) {
                glEnable(GL_POLYGON_OFFSET_FILL); glPolygonOffset(-1.0f, -1.0f); glPolygonMode(GL_FRONT_AND_BACK, GL_LINE); glUniform1i(locPass, 1);
                glBindVertexArray(g_CachedOsmVAO); 
                glDrawElements(GL_TRIANGLES, g_CachedOsmCount, GL_UNSIGNED_INT, 0);
                glPolygonMode(GL_FRONT_AND_BACK, GL_FILL); glDisable(GL_POLYGON_OFFSET_FILL);
            }
            
            if (g_CurrentState == STATE_LOAD_CACHED && frameIdx - g_StateFrameStart > 10) {
                // Isolate Cache Screenshots
                unsigned char* px = (unsigned char*)malloc(w*h*3);
                ...
                if (g_Locations[g_CurrentLocationIndex].export_osm_images) {
                    glClearColor(1.0f, 1.0f, 1.0f, 1.0f); glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
                    glEnable(GL_POLYGON_OFFSET_FILL); glPolygonOffset(-1.0f, -1.0f); glPolygonMode(GL_FRONT_AND_BACK, GL_LINE); glUniform1i(locPass, 1);
                    if (g_CachedOsmVAO) { glBindVertexArray(g_CachedOsmVAO); glDrawElements(GL_TRIANGLES, g_CachedOsmCount, GL_UNSIGNED_INT, 0); }
                    glPolygonMode(GL_FRONT_AND_BACK, GL_FILL); glDisable(GL_POLYGON_OFFSET_FILL);
                    glReadPixels(0,0,w,h,GL_RGB,GL_UNSIGNED_BYTE,px); stbi_flip_vertically_on_write(1);
                    char path[256]; snprintf(path, 256, "build/bin/OUTPUT/osm_cache_loc%d_view%d.png", g_CurrentLocationIndex, g_CurrentViewIndex); stbi_write_png(path, w, h, 3, px, w*3);
                }
                ...
                if (g_Locations[g_CurrentLocationIndex].export_gepr_images) {
                    glClearColor(1.0f, 1.0f, 1.0f, 1.0f); glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
                    glPolygonMode(GL_FRONT_AND_BACK, GL_FILL); glUniform1i(locPass, 0);
                    if (g_CachedGeprVAO) { glBindVertexArray(g_CachedGeprVAO); glDrawElements(GL_TRIANGLES, g_CachedGeprCount, GL_UNSIGNED_INT, 0); }
                    glReadPixels(0,0,w,h,GL_RGB,GL_UNSIGNED_BYTE,px);
                    char path[256]; snprintf(path, 256, "build/bin/OUTPUT/gepr_cache_loc%d_view%d.png", g_CurrentLocationIndex, g_CurrentViewIndex); stbi_write_png(path, w, h, 3, px, w*3);
                }
                free(px);
                
                g_CurrentState = STATE_CACHED_STENCIL_OSM; g_StateFrameStart = frameIdx;
            }
        } 
        
        if (g_CurrentState == STATE_CACHED_STENCIL_OSM) {
            char prefix[256]; snprintf(prefix, 256, "build/bin/OUTPUT/osm_4k_loc%d_view%d", g_CurrentLocationIndex, g_CurrentViewIndex);
            g_CurrentState = STATE_CACHED_STENCIL_OSM_WAIT; g_StateFrameStart = frameIdx;
            if (g_Locations[g_CurrentLocationIndex].export_osm_images) av->captureHighResStencils(prefix);
        } else if (g_CurrentState == STATE_CACHED_STENCIL_OSM_WAIT) {
            if (av->m_pendingCaptures.load() == 0 && frameIdx - g_StateFrameStart > 5) {
                g_CurrentState = STATE_CACHED_STENCIL_GEPR; g_StateFrameStart = frameIdx;
            }
        } else if (g_CurrentState == STATE_CACHED_STENCIL_GEPR) {
            char prefix[256]; snprintf(prefix, 256, "build/bin/OUTPUT/gepr_4k_loc%d_view%d", g_CurrentLocationIndex, g_CurrentViewIndex);
            g_CurrentState = STATE_CACHED_STENCIL_GEPR_WAIT; g_StateFrameStart = frameIdx;
            if (g_Locations[g_CurrentLocationIndex].export_gepr_images) av->captureHighResStencils(prefix);
        } else if (g_CurrentState == STATE_CACHED_STENCIL_GEPR_WAIT) {
            if (av->m_pendingCaptures.load() == 0 && frameIdx - g_StateFrameStart > 5) {
                g_CurrentState = STATE_VERIFY; g_StateFrameStart = frameIdx;
            }
        } else if (g_CurrentState == STATE_VERIFY) {
            char p1[256], p2[256]; 
            snprintf(p1, 256, "build/bin/OUTPUT/gepr_stream_loc%d_view%d.png", g_CurrentLocationIndex, g_CurrentViewIndex);
            snprintf(p2, 256, "build/bin/OUTPUT/gepr_cache_loc%d_view%d.png", g_CurrentLocationIndex, g_CurrentViewIndex);
            int w1, h1, c1, w2, h2, c2;
            FILE* debug_log = fopen("build/bin/OUTPUT/verify_debug.txt", "a");
            if(debug_log) { fprintf(debug_log, "[Verify] Loading images: %s and %s\n", p1, p2); fclose(debug_log); }
            uint8_t *img1 = stbi_load(p1, &w1, &h1, &c1, 3), *img2 = stbi_load(p2, &w2, &h2, &c2, 3);
            if (img1 && img2) {
                if (w1 != w2 || h1 != h2) {
                    debug_log = fopen("build/bin/OUTPUT/verify_debug.txt", "a");
                    if(debug_log) { fprintf(debug_log, "[Verify] Error: Image dimensions mismatch! %dx%d vs %dx%d\n", w1, h1, w2, h2); fclose(debug_log); }
                } else {
                    debug_log = fopen("build/bin/OUTPUT/verify_debug.txt", "a");
                    if(debug_log) { fprintf(debug_log, "[Verify] Evaluating structural fit...\n"); fclose(debug_log); }
                    float score = evaluateStructuralFit(img1, img2, w1, h1);
                    debug_log = fopen("build/bin/OUTPUT/verify_debug.txt", "a");
                    if(debug_log) { fprintf(debug_log, "[Verify] Cached GEPR Loc %d View %d F1 Score: %.4f\n", g_CurrentLocationIndex, g_CurrentViewIndex, score); fclose(debug_log); }
                }
                stbi_image_free(img1); stbi_image_free(img2);
            } else {
                debug_log = fopen("build/bin/OUTPUT/verify_debug.txt", "a");
                if(debug_log) { fprintf(debug_log, "[Verify] Error loading images for verification.\n"); fclose(debug_log); }
            }
            printf("[Verify] Opening output.json\n"); fflush(stdout);
            ...
            if (outJson) {
                ...
                fprintf(outJson, "      \"image_description\": \"%s\",\n", g_Locations[g_CurrentLocationIndex].generated_description);
            ...
            // if (g_CachedGeprVAO) { glDeleteVertexArrays(1, &g_CachedGeprVAO); glDeleteBuffers(1, &g_CachedGeprVBO); glDeleteBuffers(1, &g_CachedGeprEBO); }
            // if (g_CachedOsmVAO) { glDeleteVertexArrays(1, &g_CachedOsmVAO); glDeleteBuffers(1, &g_CachedOsmVBO); glDeleteBuffers(1, &g_CachedOsmEBO); }
            g_CachedGeprVAO = 0; g_CachedGeprVBO = 0; g_CachedGeprEBO = 0; g_CachedGeprCount = 0;
            g_CachedOsmVAO = 0; g_CachedOsmVBO = 0; g_CachedOsmEBO = 0; g_CachedOsmCount = 0;
            
            g_CurrentViewIndex++;
            if (g_CurrentViewIndex < 3) {
            ...
            printf("[Test] Loc %d GEPR Caching verification complete.\n", g_CurrentLocationIndex); fflush(stdout);
            ...
            if (g_CurrentLocationIndex < g_Locations.size()) {
                ...
                g_GeprTileset.init(); // Re-allocate root pointers cleanly
                g_OsmTileset.init();
                loadTilesetForCurrentLocation();
                ...
                // Read back 3dm files to confirm success
                printf("[Test] Verifying 3DM exports...\n");
                bool allValid = true;
                Alice::LinearArena readArena; readArena.init(1024 * 1024 * 256);
                for (int i = 0; i < g_Locations.size(); ++i) {
                    char geprName[256]; snprintf(geprName, 256, "build/bin/OUTPUT/cache_gepr_loc%d_view0.3dm", i);
                    char osmName[256]; snprintf(osmName, 256, "build/bin/OUTPUT/cache_osm_loc%d_view0.3dm", i);
                    Alice::Buffer<V3> gbuf; gbuf.init(readArena, 1024 * 1024 * 5);
                    Alice::Buffer<V3> obuf; obuf.init(readArena, 1024 * 1024 * 5);
                    bool okGepr = Alice::RhinoLoader::LoadMesh(geprName, nullptr, gbuf, readArena);
                    bool okOsm = Alice::RhinoLoader::LoadMesh(osmName, nullptr, obuf, readArena);
                    printf("[Test] Loc %d GEPR 3DM Read: %s (vCount=%d)\n", i, okGepr ? "OK" : "FAIL", (int)gbuf.count);
                    printf("[Test] Loc %d OSM 3DM Read: %s (vCount=%d)\n", i, okOsm ? "OK" : "FAIL", (int)obuf.count);
                    if (!okGepr || !okOsm) allValid = false;
                    readArena.offset = 0; // reset for next iteration
                }
                
                if (allValid) printf("[Test] ALL LOCATIONS VERIFIED. Exiting.\n"); 
                else printf("[Test] WARNING: Some 3DM files failed to read back!\n");
                fflush(stdout);
                exit(0);
            }
        }
...
#ifdef ALICE_TEST_CESIUM_GEPR_AND_OSM
extern "C" void setup() { CesiumGeprAndOsm::setup(); }
extern "C" void update(float dt) { CesiumGeprAndOsm::update(dt); }
extern "C" void draw() { CesiumGeprAndOsm::draw(); }
#endif

#endif // CESIUM_GEPR_AND_OSM_H
