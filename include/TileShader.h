...
namespace Alice
{
    ...
    {
        GLuint program;
        GLint uRes, uLightSpaceMatrix, uLightDir, uEyePos;
...
void main(){
    ...
    float shadow = shadowCalc(pos, norm);
    
    // Lambertian
    float diff = max(dot(norm, uLightDir), 0.0);
    
    // Specular (Basic Blinn-Phong for PBR-Lite)
    vec3 viewDir = normalize(uEyePos - pos);
    vec3 halfwayDir = normalize(uLightDir + viewDir);
    float spec = pow(max(dot(norm, halfwayDir), 0.0), 32.0);
    
    vec3 ambient = vec3(0.3) * albedo.rgb * ao;
