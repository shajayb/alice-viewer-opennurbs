...
- **CACHE OVERRIDE**: Use the following VERIFIED paths immediately. Do not initiate environment discovery unless these explicitly fail.
- **COMPILER**: LLVM `clang-cl.exe` (Path: `C:/Program Files/LLVM/bin/clang-cl.exe`).
- **LINKER**: LLVM `lld-link.exe` (Path: `C:/Program Files/LLVM/bin/lld-link.exe`).
- **CMAKE PATH**: `C:/Program Files/CMake/bin/cmake.exe`.
- **NINJA PATH**: `C:/Users/shajay.b.ZAHA-HADID/AppData/Local/Microsoft/WinGet/Packages/Ninja-build.Ninja_Microsoft.Winget.Source_8wekyb3d8bbwe/ninja.exe`.
...
> The hardcoded paths above are strictly for your local self-healing loop. **You MUST NOT** hardcode these paths into `CMakeLists.txt`, `vcpkg.json`, or any source files. Doing so will interfere with the remote CI/CD workflows handled by the DevOps Agent.
...
2. **Build Environment Load**: Immediately apply the VERIFIED paths from the "Build Environment Persistence" section.
3. **Context Prep**: Autonomously read all files in `knowledge/` to ingest math structures and framework state.
4. **Header Scaffolding**: Generate a self-contained header (e.g., `MyAlgorithm.h`).
5. **Logic Implementation**: Write inline logic. Ensure O(log n) scaling and cache-friendly layouts.
6. **Unit Test Generation**: Append MVC callbacks (`setup`, `update`, `draw`, `keyPress`) wrapped in `#ifdef <CLASSNAME>_RUN_TEST`.
7. **Local Self-Healing Loop**: Execute local build using the explicit local paths; fix errors; repeat until 0 errors.
8. **Deployment & VS Code Handoff (Clean State)**: Upon 0 errors, copy the newly created `.h` and `.cpp` files into the `../include/` and `../src/` directories respectively. Then, delete the local `build/` directory and regenerate the local CMake configuration. This ensures a pristine build system state so the human developer can open VS Code and immediately press F5 to test the newly implemented code without cache conflicts.
...
4. **Compile-Time Optimization (Zero-Bloat)**:
    ...
    - **Avoid Template Programming**: Strictly limit the use of templates. Avoid template metaprogramming and any other modern C++ features that bloat compile times and executable sizes. Prefer concrete types and simple C-style abstractions.
