...
namespace CesiumGEPR {
    ...
    struct Tileset {
        ...
        void traverse(Tile* node, const std::string& currentBase, int depth) {
            ...
            // Use exact target ECEF for Rio
            double tx = -3738507.0, ty = -3912151.0, tz = -3366838.0;
            double dx = node->centerEcef.x - tx;
            double dy = node->centerEcef.y - ty;
            double dz = node->centerEcef.z - tz;
            double distToTarget = sqrt(dx*dx + dy*dy + dz*dz);

            bool containsTarget = false;
            if (node->region[0] > -179.0) {
                double tLat = -22.951916 * 0.0174532925, tLon = -43.210487 * 0.0174532925;
                if (tLon >= node->region[0] && tLon <= node->region[2] &&
                    tLat >= node->region[1] && tLat <= node->region[3]) containsTarget = true;
            } else {
                containsTarget = (distToTarget < node->radius * 1.5);
            }

            if (!containsTarget && depth > 2) return;

            printf("[DRILL] depth=%d, distToTarget=%.1f, radius=%.1f, isImplicit=%d, children=%u, content=%s\n", 
                   depth, distToTarget, node->radius, node->isImplicit, node->childrenCount, node->contentUri);
            ...
            if (node->childrenCount > 0) {
                uint32_t best = (uint32_t)-1; double dmin = 1e30;
                for (uint32_t i = 0; i < node->childrenCount; ++i) {
                    double tdx = node->children[i]->centerEcef.x - g_TargetEcef.x;
                    double tdy = node->children[i]->centerEcef.y - g_TargetEcef.y;
                    double tdz = node->children[i]->centerEcef.z - g_TargetEcef.z;
                    double d = sqrt(tdx*tdx + tdy*tdy + tdz*tdz);
                    
                    // Only consider children that actually contain the target (with a generous overlap factor)
                    if (d < node->children[i]->radius * 1.5 || d < 50000.0) { // Or if we are very close
                        if (d < dmin) { dmin = d; best = i; }
                    }
                }
                
                // If no child strictly contains it, fall back to the absolute closest one to prevent stalling
                if (best == (uint32_t)-1) {
                    for (uint32_t i = 0; i < node->childrenCount; ++i) {
                        double tdx = node->children[i]->centerEcef.x - g_TargetEcef.x;
                        double tdy = node->children[i]->centerEcef.y - g_TargetEcef.y;
                ...
                if (best != (uint32_t)-1) {
                    traverse(node->children[best], myBase, depth + 1);
                }
            }
        }
