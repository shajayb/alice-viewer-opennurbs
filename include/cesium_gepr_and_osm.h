...
namespace CesiumGEPROSM {
    ...
    static void initShaders() {
        ...
        const char* fs = 
            ...
            "in vec3 vWorldPos;\n"
            "void main() {\n"
            "    vec3 dxW = dFdx(vWorldPos);\n"
            "    vec3 dyW = dFdy(vWorldPos);\n"
            "    vec3 N = cross(dxW, dyW);\n"
            "    float len = length(N);\n"
            "    if (len > 0.0001) N = N / len;\n"
            "    else N = vec3(0.0, 0.0, 1.0);\n"
            "    if (N.z < 0.0) N = -N;\n" // ENU: Z is UP
            "    float diffuse = max(dot(N, normalize(vec3(0.5, 0.8, 1.0))), 0.0);\n" // Light slightly from South
            "    float ambient = 0.4;\n"
            "    float height = clamp((vWorldPos.z + 50.0) / 150.0, 0.0, 1.0);\n"
            "    vec3 baseColor = mix(vec3(0.2, 0.25, 0.3), vec3(0.9, 0.95, 1.0), height);\n"
            "    vec3 color = baseColor * (ambient + diffuse * 0.6);\n"
            "    float edgeDetect = length(dFdx(N)) + length(dFdy(N));\n"
            "    float edgeOcclusion = exp(-edgeDetect * 2.0);\n"
            "    color *= mix(0.4, 1.0, edgeOcclusion);\n"
            "    color = pow(color, vec3(1.0/2.2));\n"
            "    float near = 0.1;\n"
            "    float far = 5000.0;\n"
            "    float dist = -vVP.z;\n"
            "    float fog = clamp((dist - 1200.0) / 1500.0, 0.0, 1.0);\n"
            "    f_Color = vec4(mix(color, vec3(0.85, 0.88, 0.9), fog), 1.0);\n"
